...
function AddCardCtrl($scope, $rootScope, $cookieStore, $location, $http, $route, $routeParams, phoneNumberTypes, stateCodes, fileSizeInfoContent, Busidex, $timeout, Analytics) {
    ...
    $scope.$on('fileSelected', function (event, args) {
        ...
        if (!$scope.IsBound) {
            $scope.$apply(function () {
                ...
                if (!valid) {
                    window.alert('Sorry, only jpg, gif, png and bmp files are supported');
                    return false;
                }
    ...
    var getModel = function () {
        ...
        $http({
            ...
            model.Model.Notes = window.unescape(model.Model.Notes);
            ...
            $scope.Save = function () {
                ...
                $http({
                    ...
                    if (status === 400) {
                        ...
                        window.alert('There was a problem saving your card.');
                    }
                });
                return false;
            };
            ...
            $scope.resultData = data;
            window.alert('Getting card failed.');
        });
    };
