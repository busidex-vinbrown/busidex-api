...
BEGIN
    ...
    IF ((@TablesToDeleteFrom & 1) <> 0 AND
        (EXISTS (SELECT name FROM sysobjects WHERE (name = N'vw_busidex_MembershipUsers') AND (type = 'V'))))
    BEGIN
    ...
    IF ((@TablesToDeleteFrom & 2) <> 0  AND
        (EXISTS (SELECT name FROM sysobjects WHERE (name = N'vw_busidex_UsersInRoles') AND (type = 'V'))) )
    BEGIN
    ...
    IF ((@TablesToDeleteFrom & 4) <> 0  AND
        (EXISTS (SELECT name FROM sysobjects WHERE (name = N'vw_busidex_Profiles') AND (type = 'V'))) )
    BEGIN
    ...
    IF ((@TablesToDeleteFrom & 1) <> 0 AND
        ...
        (EXISTS (SELECT UserId FROM dbo.busidex_Users WHERE @UserId = UserId)))
    BEGIN
