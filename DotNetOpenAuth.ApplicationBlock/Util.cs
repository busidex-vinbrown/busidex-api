...
namespace DotNetOpenAuth.ApplicationBlock
{
    ...
    public static class Util
    {
        ...
        internal static void AddOutgoingHttpRequestTransform(this DotNetOpenAuth.Messaging.Channel channel, Action<HttpWebRequest> action)
        {
            if (channel == null)
            {
        ...
        {
            Uri callback = DotNetOpenAuth.Messaging.MessagingUtilities.GetRequestUrlFromContext().StripQueryArgumentsWithPrefix("oauth_");
            return callback;
        }
        ...
        /// </summary>
        private class WrappingWebRequestHandler : DotNetOpenAuth.Messaging.IDirectWebRequestHandler
        {
            /// <summary>
            /// The handler being wrapped.
            /// </summary>
            private readonly DotNetOpenAuth.Messaging.IDirectWebRequestHandler wrappedHandler;
            ...
            /// <param name="action">The action to perform on outgoing HTTP requests.</param>
            internal WrappingWebRequestHandler(DotNetOpenAuth.Messaging.IDirectWebRequestHandler wrappedHandler, Action<HttpWebRequest> action)
            {
            ...
            /// </returns>
            public bool CanSupport(DotNetOpenAuth.Messaging.DirectWebRequestOptions options)
            {
            ...
            /// a single exception type for hosts to catch.</para>
            /// </remarks>
            public Stream GetRequestStream(HttpWebRequest request, DotNetOpenAuth.Messaging.DirectWebRequestOptions options)
            {
                this.action(request);
            ...
            /// </remarks>
            public DotNetOpenAuth.Messaging.IncomingWebResponse GetResponse(HttpWebRequest request)
            {
            ...
            /// value, if set, should be Closed before throwing.</para>
            /// </remarks>
            public DotNetOpenAuth.Messaging.IncomingWebResponse GetResponse(HttpWebRequest request, DotNetOpenAuth.Messaging.DirectWebRequestOptions options)
            {
                // If the request has an entity, the action would have already been processed in GetRequestStream.
                if (request.Method == "GET")
                {
