...
namespace Busidex.Services
{
    public class CardRepository : RepositoryBase, ICardRepository
    {
        ...
        public void AddToMyBusidex(long id, long userId)
        {
            ...
            if (account != null)
            {
                template.Populate(null, account, card);
        ...
        public CardDetailModel GetCardDetail(DomainModels.Card card)
        {
        ...
        public DomainModels.Card GetCardById(long cardId, long userId = 0)
        {
        ...
        {
			DomainModels.Card card = _dao.GetCardById(model.CardId);
            ...
            model.Addresses = card.Addresses ?? new List<DomainModels.CardAddress>();
        ...
        {
            var card = cardId == 1 ? new DomainModels.Card() : _dao.GetCardById(cardId, bu.UserId);
            var appSettings = new AppSettingsReader();
            card = card ?? new DomainModels.Card();
            if (card.Addresses.Count == 0)
            {
                card.Addresses.Add(new DomainModels.CardAddress());
            }
        ...
        public SearchResultModel Search(SearchResultModel model, long? userId, Tuple<double, double> altAddress)
        {
            ...
            var cards = new List<DomainModels.Card>();
        ...
        private IEnumerable<DomainModels.Card> FilterSearchResults(List<DomainModels.Card> cards, IEnumerable<UserCard> mybusidex, long userId)
        {
            var results = new List<DomainModels.Card>();
        ...
        public DomainModels.Group GetBusiGroupById(long groupId)
        {
            return _dao.GetGroupById(groupId);
        }

        public DomainModels.Group GetGroupByName(string groupName)
        {
            return _dao.GetGroupByName(groupName);
        }

        public List<DomainModels.Group> GetMyBusiGroups(long userId)
        {
        ...
        public void AddGroup(DomainModels.Group group, string cardIds)
        {
        ...
        public long UpdateGroup(DomainModels.Group group)
        {
        ...
        public AddOrUpdateCardErrors EditCard(DomainModels.Card cardModel, bool isMyCard, long userId, string notes)
        {
        ...
        private void NotifyUsersOfChangedCard(DomainModels.Card card)
        {
        ...
        public AddOrUpdateCardErrors CheckForCardModelErrors(DomainModels.Card cardModel, bool isMyCard)
        {
        ...
        private void SendOwnerNotificationOfSharedCard(SharedCard model, UserAccount account, DomainModels.Card card, EmailTemplate template)
        {
        ...
        private static Communication GetCommunication(SharedCard model, DomainModels.Card card, UserAccount account, EmailTemplate template)
        {
            template.Populate(model, account, card);
        ...
        public AddOrUpdateCardErrors AddCard(DomainModels.Card card, bool isMyCard, long userId, string notes, out long cardId)
        {
        ...
        {
			DomainModels.Card card = _dao.GetCardByToken(token);
