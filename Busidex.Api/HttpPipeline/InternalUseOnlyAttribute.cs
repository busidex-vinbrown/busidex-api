...
{
    /// <summary>
    /// Indicates that a controller method may be invoked only if the special X-Virgin-App-Name header is present.
    /// This header is always stripped from external requests, so only apps within the Virgin datacenter may invoke it.
    /// </summary>
    [AttributeUsage(AttributeTargets.Method)]
    public class InternalUseOnlyAttribute : ActionFilterAttribute
    {
        private const string APP_ACCESS_HEADER = "X-Virgin-App-Name";
